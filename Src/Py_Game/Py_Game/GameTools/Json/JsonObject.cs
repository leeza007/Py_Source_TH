...
using Py_Game.GameTools.Json.Serialization;

namespace Py_Game.GameTools.Json
{
	/// <summary>
	...
	public sealed class JsonObject : IEnumerable<KeyValuePair<string, JsonValue>>, IEnumerable<JsonValue>
	{
        ...
        public JsonObject Add(JsonObject key, JsonValue value)
        {
        ...
        public void Add(JsonObject key, JsonObject key2)
        {
            foreach (var data in key)
            {
                Add(data.Value);
            }
            foreach (var data in key2)
            {
                Add(data.Key, data.Value);
            }
        }
        public JsonObject Add(JsonObject key)
        {
            foreach (var data in key)
            {
                Add(data.Key, data.Value);
        ...
        public JsonObject AddSub(JsonObject key, JsonValue value)
        {
            foreach (var item in key)
            {
                this.properties.Add(item.Key, value);
            }
            return this;
        }
        /// <summary>
